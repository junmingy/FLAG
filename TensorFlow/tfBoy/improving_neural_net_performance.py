...
def construct_feature_columns(input_features):
    ...
    :param input_features: The name of the numerical input features to use
...
def train_nn_regression_model(
    ...
    :param steps: A non-zero 'int', the total number of training steps
                  ...
                  a single batch
    :param hidden_units: A 'list'of int values, specifying the number of neurons
                         in each layer
    ...
    :param training_examples: A 'DataFrame' containing one or more columns from
                              ...
                              for training
    :param training_targets: A 'DataFrame' containing exactly one columns from
    ...
    # Train the model, but do not inside a loop so that we can periodically assess
    # loss metrics
...
_ = plt.legend()  # Shows all the label
plt.show()
...
def log_normalize(serises):
    return serises.apply(lambda x:math.log(x + 1.0))

def clip(serises, clip_to_min, clip_to_max):
    return serises.apply(lambda x:(
        min(max(x, clip_to_min), clip_to_max)
    ))

def z_score_normalize(serises):
    mean = serises.mean()
    std_dv = serises.std()

    return serises.apply(lambda x:(x - mean) / std_dv)

def binary_threshold(serises, threshold):
    return serises.apply(lambda x:(1 if x > threshold else 0))


def normalize(examples_dataframe):
    # Returns a version of the input 'DataFrame' that has all its features normalized
    processed_feature = pd.DataFrame()

    processed_feature["households"] = log_normalize(examples_dataframe["households"])
    processed_feature["median_income"] = log_normalize(examples_dataframe["median_income"])
    processed_feature["total_bedrooms"] = log_normalize(examples_dataframe["total_bedrooms"])
    ...
    processed_feature["housing_median_age"] = linear_scale(examples_dataframe["housing_median_age"])

    processed_feature["population"] = linear_scale(clip(examples_dataframe, 0, 5000))
    processed_feature["rooms_per_person"] = linear_scale(clip(examples_dataframe, 0, 5))
    processed_feature["total_rooms"] = linear_scale(clip(examples_dataframe, 0, 10000))
...
def location(examples_dataframe):
    # Return a version of the input 'DataFrame' that keeps only the
...
location_dataframe = location(preprocess_features(california_housing_dataframe))
location_training_examples = location_dataframe.head(12000)
location_validation_examples = location_dataframe.tail(5000)
